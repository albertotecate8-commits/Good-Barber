...
export function renderAppointmentCard(a, { showBarber = false } = {}) {
  return `
    <div class="card appointment-card" data-appt="${a.id}">
      <div class="flex-between">
        <strong>${timeHHMM(a.starts_at)}${a.ends_at ? " – " + timeHHMM(a.ends_at) : ""}</strong>
        <span class="badge ${STATUS_BADGE[a.status] || "badge-neutral"}">${STATUS_LABEL[a.status] || a.status}</span>
      </div>
      ${showBarber ? `<div class="text-muted mt-8">${escapeHtml(a.barbers?.name || "—")}</div>` : ""}
      <div class="mt-8" style="font-weight:700">${escapeHtml(a.client_name)}</div>
      <div class="text-muted">${a.client_phone ? escapeHtml(a.client_phone) : "Sin teléfono"}</div>
      <div class="mt-8">${escapeHtml(a.service_name)} · ${a.duration_minutes} min · ${formatCents(a.price_cents)}</div>
      <div class="text-muted mt-8" style="font-size:12px">Código: ${escapeHtml(a.booking_code)}</div>
      ${a.notes ? `<div class="text-muted mt-8">Nota: ${escapeHtml(a.notes)}</div>` : ""}
...
export function wireAppointmentActions(list, appointments, onDone) {
  appointments.forEach((a) => {
    ...
    if (!slot) return;
    const buttons = [];
    if (a.status === "pending") buttons.push(`<button class="btn btn-ghost btn-sm" data-confirm="${a.id}">Confirmar</button>`);
    if (a.status === "pending" || a.status === "confirmed") buttons.push(`<button class="btn btn-ghost btn-sm" data-cancel="${a.id}">Cancelar</button>`);
    if (a.status === "confirmed") buttons.push(`<button class="btn btn-primary btn-sm" data-complete="${a.id}">Completar</button>`);
